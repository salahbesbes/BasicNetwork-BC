...
installChaincode(){
    ...
    # ? this is not endorsing peer we dont have to install chain code  
    # setGlobalsForPeer1Org1
    # peer lifecycle chaincode install ${CC_NAME}.tar.gz
...
checkCommitReadyness(){
    ...
    setGlobalsForPeer0Org1
    peer lifecycle chaincode checkcommitreadiness \
    --channelID $CHANNEL_NAME --peerAddresses localhost:7051 \
    --tlsRootCertFiles $PEER0_ORG1_CA --name ${CC_NAME} \
...
chaincodeInvokeInit(){
    ...
    --ordererTLSHostnameOverride orderer.example.com \
    --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA \
    -C $CHANNEL_NAME -n ${CC_NAME} --peerAddresses localhost:7051 \
    --tlsRootCertFiles $PEER0_ORG1_CA --peerAddresses localhost:9051 \
    --tlsRootCertFiles $PEER0_ORG2_CA --isInit -c '{"Args":[]}'
